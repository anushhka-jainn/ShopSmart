// The fruits_list variable is an array of objects. Each object represents a fruit with various properties like brand, product information, discount, MRP (Maximum Retail Price), name, URL to the product image, etc.
...
// The code interacts with the browser's local storage. It stores the fruits_list array in the local storage as "fruitsList" and retrieves it later.
// It then fetches the element with the id "listoffruits" from the HTML document and calls the displayFruits() function, which populates the list of fruits on the webpage.
  ...
  var count = 0;
  // The displayFruits() function iterates through each fruit object in the fruits_list, creates HTML elements dynamically (image, name, price, add to cart button), and appends them to the "listoffruits" element.
  ...
  // The code handles adding items to the shopping cart. It listens for clicks on the "Add to Cart" buttons, allowing users to increment or decrement the quantity of items they want to add to the cart.
  ...
  // The toChart() function redirects the user to a "mycart.html" page when they click the "Checkout" button. It also filters out items with a quantity of 0 and updates the cart items in the local storage.
  function toChart() {
  ...
  // The code provides sorting functionality based on price (low to high, high to low) and discount percentage. When a sort option is selected, the handleSort() function sorts the fruits_list array accordingly and calls displayFruits() to update the display.
  ...
  // Lastly, the code applies an active effect to the sorting buttons, highlighting the currently selected option.

// Overall, this code creates a dynamic webpage for displaying a list of fruits, allowing users to add items to their cart and sort the list based on various criteria.
